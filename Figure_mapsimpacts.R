...
temp_impact <- get_witch("damfrac_type") %>%
...
temperature_maps <- temp_impact %>% 
  ...
  filter(ttoyear(t)==2100  & impacts !="bhm" ) %>% 
  inner_join(countries_map) %>% 
  left_join(reg %>% filter(iso3!='ATA')) %>% 
  ggplot() +
  geom_polygon(aes(x = long, y = lat,group = group, fill = -value*100),color='grey50',size=.05) +
  geom_polygon(data= . %>% filter(n==n_to_name[nsrm]), 
               aes(x = long, y = lat,group = group, fill = -value*100),color='black',size=.2) +
  geom_point(data=Z_SAI %>% 
               inner_join(sanitized_names) %>%
               mutate(ninj=ifelse(str_detect(inj,"S"),- as.numeric(str_remove(inj,"S")), as.numeric(str_remove(inj,"N") ))) %>%
               filter(ttoyear(t)==2100 & value != 0 & impacts !="bhm"),
  ...
  facet_grid(ordered(Scenario,c("Free-riding","Mitigation","Mitigation + SAI","China","USA","Brazil","India"))~impacts)


prec_impact <- get_witch("damfrac_type") %>%
  mutate(source=case_when(str_detect(d,"temp")~"temp",
                          str_detect(d,"prec")~"prec",
                          str_detect(d,"spill")~"spill",
                          .default="others")) %>%
...
precipitation_maps <- prec_impact %>% 
  ...
  geom_polygon(data= . %>% filter(n==n_to_name[nsrm]), 
               aes(x = long, y = lat,group = group, fill = - value),color='black',size=.2) +
  geom_point(data=Z_SAI %>% 
               inner_join(sanitized_names) %>%
               mutate(ninj=ifelse(str_detect(inj,"S"),- as.numeric(str_remove(inj,"S")), as.numeric(str_remove(inj,"N") ))) %>%
               filter(ttoyear(t)==2100 & value != 0 & impacts !="bhm"),
...
spill_impact <- get_witch("damfrac_type") %>%
  mutate(source=case_when(str_detect(d,"temp")~"temp",
                          str_detect(d,"prec")~"prec",
                          str_detect(d,"spill")~"spill",
                          .default="others")) %>%
  group_by(t,n,file,source) %>%
  summarise(value=sum(value)) %>% filter(source=="spill" & t==18) 

spillover_maps <- spill_impact %>% 
  inner_join(sanitized_names) %>%
  filter(ttoyear(t)==2100  & impacts !="bhm") %>%
  left_join(reg %>% filter(iso3!='ATA')) %>% 
  ggplot() +
...
gdploss_maps <- gdploss %>% 
  ...
  geom_polygon(aes(x = long, y = lat,group = group, fill = -value),size=.05,color="grey50") +
  geom_polygon(data= . %>% filter(n==n_to_name[nsrm]), 
               aes(x = long, y = lat,group = group, fill = -value),color='black',size=.4) +
  geom_point(data=Z_SAI %>% 
               inner_join(sanitized_names) %>%
               mutate(ninj=ifelse(str_detect(inj,"S"),- as.numeric(str_remove(inj,"S")), as.numeric(str_remove(inj,"N") ))) %>%
               filter(ttoyear(t)==2100 & !Scenario %in% c("Free-riding") & value != 0 & impacts !="bhm"),
             aes(x=-170, y = ninj, size= value ), shape=21, color="black", fill=NA ) +
  scale_fill_gradient2() +
  theme_void()+ 
  guides(size = FALSE) +
