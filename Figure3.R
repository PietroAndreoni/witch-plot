...
temperature_maps <- TEMP %>% 
  ...
  filter(ttoyear(t)==2100 & impacts==imp_select & ci_imp==ci_sel & dsc_p==downscaling_sel &  Scenario %in% c("Mitigation","Mitigation + SAI","USA","India","Brazil","China")  ) %>% 
...
#  mutate(val=temp-base_temp) %>% 
  ...
  geom_point(data=Z_SAI %>% 
               ...
               filter(ttoyear(t)==2100 & ci_imp==ci_sel & dsc_p==downscaling_sel & impacts==imp_select & Scenario %in% c("Mitigation","Mitigation + SAI","USA","India","Brazil","China") & value != 0),
