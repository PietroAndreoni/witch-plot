...
temp  <- emulator_data["srm_temperature_response_pop"] %>% 
  mutate(w="pop") %>%
...
prec  <- emulator_data["srm_precip_response_pop"] %>% 
  mutate(w="pop") %>%
...
f1a <- ggplot(temp %>% filter(inj_lat %in% c(-30,0,30) ) ) +
  geom_point(data=.%>%
               filter(se=="obs" & w==downscaling_sel),
             aes(x=meanlat,
                 y=value,
  ...
  geom_smooth(data=.%>%
              ...
              method = "loess",
              linewidth=2)+
  geom_smooth(data=unique(inner_join(reaction_t_srm %>% filter(inj_lat==0),temp %>% filter(se=="obs" & w==downscaling_sel) %>% select(n,meanlat))) %>%
                inner_join(pop2020),  
              aes(x=as.numeric(meanlat), 
                  y=t_eq,
                  weight=pop2), 
...
f1b <- ggplot(prec %>% inner_join(sd_prec) %>%
  ...
  geom_point(data=unique(inner_join(reaction_p_srm %>% filter(inj_lat==0),prec %>% select(n,meanlat))) %>% inner_join(sd_prec),  
             ...
             color="black", 
             alpha=0.1) +
  geom_point(data=.%>%filter(se=="obs" & w==downscaling_sel),aes(x=meanlat,
                                                        y=value/sd,
...
c <- gdploss %>% 
  inner_join(sanitized_names) %>%
  filter(ttoyear(t)==2100 & Scenario %in% c("Free-riding", "Mitigation") & impacts==imp_select & ci_imp==ci_sel & dsc_p==downscaling_sel) %>%
