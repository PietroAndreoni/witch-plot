rm(list = ls())
witch_folder = "../Results_srm/noprec13112023" #Where you're RICE/DICE/RICE50x code is located
...
.x %>% mutate(file=str_remove(file,"results")) %>%
  mutate(
  COOP=case_when(str_detect(file,"noncoop")~"noncoop",
  ...
  pimp=case_when(str_detect(file,"P0")~"zero",
                 ...
                 .default = "no SRM") ) %>%
    filter(nsrm!="UN Security Council and BRICS" & pimp!="medium") 
}
...
sanitized_names <- as.data.frame(unique(SRM %>% select(file)) %>% sanitize() %>% filter(!(nsrm=="USA" & COOP=="noncoop") & nsrm!="Russia"))
...
globtemp <- ggplot(land_temp %>%
  ...
  geom_line(aes(x=ttoyear(t),
                y=value-land_temp0,
                color=interaction(nsrm,COOP)), 
            linewidth=1.2) +
#  scale_color_manual(values=coop_palette,name="Precipitation impacts") +
...
#  scale_color_manual(values=coop_palette,name="Precipitation impacts") +
  xlab("") + ylab("Local temperature variation \n [°C relative to preindustrial]") + 
  theme_pubr() 
...
precip2100 <- DPRECIP_SRM %>% rename(prec=value) %>% 
  ...
  inner_join(pop %>% rename(pop=value)) %>%
  ggplot() +
  geom_point(aes(x=meanlat,y=prec*100,color=nsrm),size=0.5,alpha=0.5) + 
...
#  scale_color_manual(values=coop_palette,name="Precipitation impacts") +
  ...
  theme_pubr() 
fig2_coops <- ggarrange(regtemp2100,precip2100,common.legend = TRUE)
ggsave("fig2_coops.png",plot=fig2_coops,width=18, height=9, units="cm")
...
fig3_coops <- ggarrange(void,damagescoop,void,widths = c(0.2,1,0.2),nrow=1)
ggsave("fig3_coops.png",plot=damagescoop,width=13, height=12, units="cm")
