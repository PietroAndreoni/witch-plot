figa <- W_SAI %>%
  ...
  geom_line(aes(x=ttoyear(t),
                ...
                color=Scenario),
            linewidth=1) +
  ggrepel::geom_text_repel(data=.%>% inner_join(Z_SAI %>% 
    inner_join(sanitized_names) %>%
...
figb <- TATM %>%
  inner_join(sanitized_names) %>%
  filter(ttoyear(t)<=2100) %>%
  ggplot(aes(x=ttoyear(t),y=value,color=Scenario,group=file)) +
  ...
  geom_line(data=TATM_GHG %>%
              ...
              mutate(Scenario=case_when(nsrm=="no SRM" & COOP=="coop" ~ "Optimal, no SAI",
                                        ...
                                        .default=nsrm) ) %>%
              filter(ttoyear(t)<=2100 & nsrm!="no SRM" & COOP=="noncoop"),
            aes(x=ttoyear(t),y=value,color=Scenario,group=file), linetype=2, linewidth=1) +
  scale_color_manual(values=regpalette_srm,name="Scenario") +
