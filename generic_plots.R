...
sd_prec <- srm_regional_data["precipitation_hist"] %>% group_by(n) %>% pivot_wider() %>% summarise(sd=sd/mean) %>%
  bind_rows(data.frame(n=setdiff(unique(pop$n),unique(srm_regional_data["precipitation_hist"]$n)),sd=NA)) %>%
  ungroup() %>%
...
pop2020 <- pop %>% filter(ssp=="ssp2" & t==2) %>% select(n,pop)
...
f3b <- ggplot(prec %>% filter(inj_lat %in% c(-30,0,30))) +
  ...
  facet_grid(.~inj_lat,) +
  scale_color_viridis_d(name = "Injection latitude") + xlab("Average country latitude") + ylab("Variation of average daily precipitation [STD]")
