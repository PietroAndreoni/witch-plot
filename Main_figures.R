...
fig1a <- FLOWS %>% mutate(net = transfer + gentax + cdrcost + ctx + abcost + cdrrev) %>% 
  ...
  mutate(Source=case_when( Flow=="abcost" ~ "Emission reduction costs",
                           Flow=="cdrcost" ~ "Cost of NETs",
                           Flow=="ctx" ~ "Carbon tax payments",
                           Flow=="gentax" ~ "Income taxes",
...
fig1b <- ggplot(ineq_weights %>% filter(ttoyear(t) %in% c(2075) & ineq_elast %in% c("carbon_rev","abatement","tax") ) %>%
                  mutate(ineq_elast=case_when(ineq_elast=="carbon_rev" ~ "NETs profits",
                                              ineq_elast=="abatement" ~ "Carbon tax, ER costs and redistribution",
                                              ineq_elast=="tax" ~ "Income taxes" )) %>% 
  ...
  geom_point(data=. %>%group_by(ineq_elast,t,ssp,dist) %>% summarise(med=median(value)),
             aes(x=dist,y=med,color=ineq_elast),size=2,shape=21,fill="white") + 
...
main1 <- inner_join(share,
  ...
  mutate(breaks=arules::discretize(use/(use+abate)*100,breaks=4)) %>% ggplot() +
...
fig3a <- ggdraw() +
  draw_plot(main1) +
  draw_plot(map1, x = 0.13, y = 0.95, width = .35, height = -.28)
...
main <- share %>%
  ...
  geom_segment(x=2045,xend=2045,y=0,yend=0.45,linetype=2) +
...
insert <- inner_join(PROF_CDR,E_NEG) %>%
  ...
  geom_ribbon(data=.%>%group_by(t) %>% summarise(cost_min=quantile((value-cost_avg*1e3)/value,0.33 ),cost_max=quantile((value-cost_avg*1e3)/value,0.66)),
              aes(x=ttoyear(t),ymin=cost_min*100,ymax=cost_max*100),alpha=0.2) + xlab('') + ylab('Profit margin [%]') + 
  theme_pubr() +
  theme(legend.position = "bottom",text = element_text(size = 7), 
...
fig3b <- ggdraw() +
  draw_plot(main) +
  draw_plot(insert, x = 0.15, y = .6, width = .35, height = .3)

fig3 <- ggarrange(fig3a,fig3b,common.legend=TRUE,labels=c("a","b"))
ggsave("fig3.pdf",width=18,height=10,dpi=300,units="cm")
...
ggsave("fig4.pdf",width=8.8,height=9,dpi=300,units="cm")
